...
def test_set_entry(request, catalog, tmp_path_factory, data_catalog_fixture):
    ...
    root_path = "catalog"
    lens = Lens(VariantCatalog, "image", data_catalog, root_path, volume_protocol_args)
    ...
    lens = Lens(VariantCatalog, "images", data_catalog, root_path, volume_protocol_args)
    ...
    for i, value in enumerate(value_list):
        ...
        assert target.tobytes() == src.tobytes()
    lens = Lens(VariantCatalog, "value", data_catalog, root_path, volume_protocol_args)
    lens.set(catalog.value)
    value = lens.get()
    assert value.path == "catalog/value"
    assert value.get() == catalog.value.get()
    lens = Lens(VariantCatalog, "values", data_catalog, root_path, volume_protocol_args)
...
def test_set_file_entry(request, catalog, tmp_path_factory, data_catalog_fixture):
    ...
    root_path = "catalog"
    lens = Lens(
        VariantCatalog, "file:image", data_catalog, root_path, volume_protocol_args
    )
    ...
    lens = Lens(
        VariantCatalog, "file:images", data_catalog, root_path, volume_protocol_args
    )
    ...
    lens = Lens(
        VariantCatalog, "file:value", data_catalog, root_path, volume_protocol_args
    )
    ...
    lens = Lens(
        VariantCatalog, "file:values", data_catalog, root_path, volume_protocol_args
    )
...
def test_get_file_entry(request, catalog, tmp_path_factory, data_catalog_fixture):
    ...
    volume_protocol_args = {"dir": {"path": storage_path}}
    root_path = "catalog"
    lens = Lens(VariantCatalog, "", data_catalog, root_path, volume_protocol_args)
    lens.set(catalog)
