...
def test_dataset():
    ...
    ds = lance.write_dataset(data, "./test.lance", schema=schema, mode="overwrite")
    # table = ds.to_table(
    ...
    print(Catalog.parse_obj(table.to_pylist()[0]))

    table = ds.to_table(columns=["values"])
    print(table.to_pylist()[0])
    print(Values.parse_obj(table.to_pylist()[0]["values"]))

    # scanner = ds.scanner(columns=[])
    # table = scanner.take([0]) # This raises NotImplementedError
    # print(table)
    ...
    table = ds.to_table(columns={"drive": "drives[1]"})
    print(Drive.parse_obj(table.to_pylist()[0]["drive"]))
    table = ds.to_table(columns={"drive": "drives[2]"})
    print(Drive.parse_obj(table.to_pylist()[0]["drive"]))
    table = ds.to_table(columns={"drive": "drives[1]['navigation'][1]"})
    print(Navigation.parse_obj(table.to_pylist()[0]["drive"]))
    # print(ds.versions())
